...
All endpoints are mounted under `/api/instances` on the control plane. All IDs
are integers assigned by the database, not UUIDs.
...
| POST | `/api/instances` | Create instance from mapping |
| GET | `/api/instances/{id}` | Get instance details |
| PUT | `/api/instances/{id}` | Update instance metadata |
| DELETE | `/api/instances/{id}` | Delete instance |
| PUT | `/api/instances/{id}/lifecycle` | Update TTL and timeout |
| PUT | `/api/instances/{id}/cpu` | Update CPU cores |
| PUT | `/api/instances/{id}/memory` | Upgrade instance memory |
| GET | `/api/instances/{id}/progress` | Get startup progress |
| GET | `/api/instances/{id}/events` | Get instance events |
| GET | `/api/instances/user/status` | Get current user's instance status |
...
|-----------|------|---------|-------------|
| snapshot_id | integer | - | Filter by snapshot_id |
  ...
  "data": [
    {
      ...
      "instance_url": "https://control-plane-graph-olap-platform.hsbc-12636856-udlhk-dev.dev.gcp.cloud.hk.hsbc/123/",
      "status": "running",
      "error_code": null,
      "error_message": null,
...
- `stack_trace` is only included in the detail view (`GET /api/instances/{id}`), not in list view, to reduce payload size.
...
> Set name and description at instance creation time via `POST /api/instances`.
...
> **Prefer TTL-based cleanup:** Set `ttl` at creation time via `POST /api/instances`
...
> parameters when calling `POST /api/instances`. Use this endpoint
...
```
PUT /api/instances/{id}/memory
```

Upgrade memory for a running instance using K8s in-place resize. Only memory
INCREASES are allowed: decreasing the limit below current usage would OOM-kill
the process.
...
| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `memory_gb` | integer | Yes | New memory limit in GiB. Must be greater than the current memory and at most 32. |
...
**Response: 400 Bad Request** (memory_gb above maximum)

```json
{
  "error": {
    "code": "VALIDATION_FAILED",
    "message": "memory_gb exceeds maximum allowed (32)",
    "details": {"requested": 64, "max_allowed": 32}
  }
}
```

**Response: 409 Conflict** (Instance not running or decrease attempted)

```json
{
  "error": {
    "code": "INVALID_STATE",
    "message": "Can only upgrade memory for running instances, and only increases are allowed",
    "details": {"current_status": "starting"}
  }
}
```
...
```
GET /api/instances/user/status
```

Returns concurrency counts and limits for the current authenticated user.
