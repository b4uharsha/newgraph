...
[ADR-128: Operational Documentation Strategy](--/operations/adr-128-operational-documentation-strategy.md) establishes that operational guarantees (SOX audit trails, segregation of duties, raw-kubectl deployment model, `graph-olap-platform/staging/production` namespace targets) apply to the **HSBC handoff artefact** — a separate deliverable built via `make hsbc` into `build/hsbc/` — and not to the Sparkling Ideas demo cluster that runs in `europe-west2` under the `hsbc-graph` GCP project. The demo cluster exists for functional validation, notebook E2E testing, and stakeholder demonstrations; it holds no real banking data, is IP-whitelisted to Sparkling Ideas office IPs only ([ADR-112](-/adr-112-remove-auth0-replace-with-ip-whitelisting.md)), and its GCS lifecycle rule deletes all contents after 7 days.
