...
The platform uses Google Secret Manager for secrets, Workload Identity for GCP access, IP whitelisting for ingress access control (ADR-112 removed Auth0 and replaced it with nginx ingress IP whitelisting and X-Username header identity), and the CyberFlow/Aqua/Nexus pipeline (image scanning, runtime protection, and artifact repository, respectively) for container supply chain governance. These operational details need to be consolidated into a single runbook.
...
6. **Compliance operations** -- SOX audit preparation, Deliverance change control integration, and evidence collection procedures.
...
- **Compliance evidence collection** maps each SOX audit requirement to a specific data source and collection command.
...
- Some procedures (e.g., CyberFlow image submissions, Deliverance change requests) depend on HSBC-internal systems not controlled by the platform team. Specifically: CyberFlow for image acquisition approval, Deliverance for SOX change control, and the HSBC SOC for incident escalation.
