...
<!-- jupyter-labs scope leak removed 2026-04-20 -->
...
|--------|---------|---------|
| Cloud SQL database password | Google Secret Manager (`database-url`) | control-plane |
| Internal API key | Google Secret Manager (`api-internal-token`) | control-plane, export-worker |
| Starburst Galaxy password | Google Secret Manager (`starburst-password`) | export-worker |
| GCS service account key | Workload Identity (no key file) | all services |
| Docker registry credentials | Workload Identity (no key file) | GKE node pool |

> **Note:** ADR-112 removed Auth0 and all OIDC infrastructure. Access control uses ingress IP whitelisting and X-Username headers. When HSBC production authentication is finalised (Azure AD auth proxy per ADR-137), add the relevant secrets to this inventory.

> **Rotation frequency:** Credential rotation cadence is set by HSBC security policy, not by this runbook.

### Secret Rotation: Cloud SQL Password
...
### Secret Rotation: Internal API Key
...
### Secret Rotation: Starburst Galaxy Password
...
3. **Audit** access logs for suspicious activity during the exposure window:
   ...
   ```
4. **Report** the incident through the HSBC security incident reporting channel per HSBC operations policy.
5. **Document** the incident: timeline, scope, actions taken, and follow-up items.

> **Change Control:** Emergency rotations follow the HSBC Deliverance emergency change process.
...
Triage findings by CVSS score. Response-time SLAs are set by HSBC security policy; the table below is for classification only.
...
4. Deploy via the HSBC Jenkins CD stage or, for manual promotion, the deploy script (subject to HSBC Deliverance change control):

   ```bash
   ./infrastructure/cd/deploy.sh <service> <image-tag>
...
Access review cadence is set by HSBC access management policy. The platform-side review steps are:

1. Export the current ingress IP whitelist and identify the owner of each IP/CIDR range.
2. Cross-reference with the authorised personnel register maintained by HSBC.
...
4. Verify service account permissions have not been expanded beyond their documented scope:
   ```bash
   for SA in control-plane export-worker; do
     echo "--- ${SA} ---"
   ...
   ```
5. Document the review outcome according to HSBC evidence retention policy.

### Segregation of Duties

Segregation of duties, approver requirements, and emergency-access controls are defined by HSBC policy. Platform-enforceable notes:

- Until the Azure AD auth proxy (ADR-137) is live, role-based technical enforcement at the application layer is limited — IP whitelisting and HSBC procedural controls are the only boundary. Entries in audit logs keyed on `X-Username` cannot provide non-repudiation until enterprise IAM is integrated.
...
Monitor for these patterns in Cloud Logging. Severity and thresholds should be tuned to HSBC security policy; the values below are indicative starting points.

| Indicator | Description |
|-----------|-------------|
| Repeated 401/403 from one IP | Brute-force or credential stuffing |
| 403 on admin endpoints from non-admin user | Privilege escalation attempt |
| Unusual hours API activity | Access outside the expected operational window |
| High volume of instance creation | More than 10 instances created by one user in 1 hour (tune per deployment) |
| API calls from non-whitelisted IPs | IP whitelist bypass or misconfiguration |
| Bulk data export by a single user | More than 5 exports in 1 hour from a single user (tune per deployment) |
| Ingress whitelist modification | Unauthorised CIDR range added to ingress annotation |
...
flowchart TD
    ...
    AUDIT --> SOC[Report per HSBC<br/>security incident process]:::process
...
4. Report through the HSBC security incident reporting channel and follow the [Incident Response Runbook](--/process/adr/operations/adr-130-incident-response-runbook.md) for formal incident lifecycle management.
...
Compliance framework selection, audit cadence, and evidence handling are owned by HSBC and are not defined in this runbook. The platform exposes the following data sources that can be drawn on for audit evidence:

| Data Source | How to Collect |
|-------------|----------------|
| Change history | `git log` on the source repository; Jenkins `gke_CI()` build logs |
| Access control (ingress whitelist) | `kubectl get ingress -n graph-olap-platform -o yaml` |
| Service account IAM bindings | `gcloud projects get-iam-policy <PROJECT>` |
| Vulnerability scan results | On-demand `gcloud artifacts docker images scan` output |
| Secret rotation history | `gcloud secrets versions list --secret=<NAME>` |
| Deployment history | `kubectl -n graph-olap-platform rollout history deployment/<service>`; Jenkins archive |

All production changes go through HSBC Deliverance change control.

---

## Hardening Checklist

Review cadence is set by HSBC security policy. Record the date and outcome for each item when the checklist is exercised.
...
- [ ] Binary Authorization is in ENFORCED mode for the HSBC `asia-east2` project (DRYRUN is not acceptable in production; document an exception with security team approval if ENFORCED cannot be enabled).
...
- [Change Control Framework](--/governance/change-control-framework.governance.md) — HSBC Deliverance change control
